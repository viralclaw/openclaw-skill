#!/usr/bin/env python3
"""Generate carousel slides from a video using ViralClaw API."""
...
def main():
    p = argparse.ArgumentParser(description="Generate carousels from video")
    p.add_argument("video_url")
    p.add_argument("--slides", type=int, default=7)
    p.add_argument("--style", default="corporate", choices=["corporate", "minimal", "bold", "dark"])
    p.add_argument("--language", default=None)
    p.add_argument("--wait", action="store_true")
    p.add_argument("--json", action="store_true")
    a = p.parse_args()

    payload = {"video_url": a.video_url, "slide_count": a.slides, "style": a.style}
    if a.language: payload["language"] = a.language

    print(f"🎠 Generating carousels from: {a.video_url}", file=sys.stderr)
    r = _request("POST", "/api/v1/generate-carousels", payload)
    job_id = r.get("job_id")
