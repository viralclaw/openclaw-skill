#!/usr/bin/env python3
"""Generate quote cards from a video using ViralClaw API."""
...
def main():
    p = argparse.ArgumentParser(description="Generate quote cards from video")
    p.add_argument("video_url")
    p.add_argument("--count", type=int, default=3)
    p.add_argument("--style", default="gradient", choices=["gradient", "minimal", "dark", "bold"])
    p.add_argument("--format", default="both", choices=["png", "zip", "both"])
    ...
    print(f"💬 Generating quote cards from: {a.video_url}", file=sys.stderr)
    r = _request("POST", "/api/v1/generate-quote-cards", payload)
    job_id = r.get("job_id")
