#!/usr/bin/env python3
"""Generate Twitter/LinkedIn threads from a video using ViralClaw API."""
...
def main():
    p = argparse.ArgumentParser(description="Generate threads from video")
    p.add_argument("video_url")
    p.add_argument("--platform", default="both", choices=["twitter", "linkedin", "both"])
    p.add_argument("--tone", default="casual", choices=["casual", "professional", "provocative"])
    p.add_argument("--language", default=None)
    p.add_argument("--wait", action="store_true")
    p.add_argument("--json", action="store_true")
    a = p.parse_args()

    payload = {"video_url": a.video_url, "platform": a.platform, "tone": a.tone}
    if a.language: payload["language"] = a.language

    print(f"🧵 Generating threads from: {a.video_url}", file=sys.stderr)
    r = _request("POST", "/api/v1/generate-threads", payload)
    job_id = r.get("job_id")
